    ...
    methods: {
        ...
        async init() {
            ...
            try {
                ...
                if (this.logged) {
                    ...
                    // Only fetch from server if has_analyzed is true AND no local data.
                    // When has_analyzed is false, skip entirely to prevent stale records
                    // from old test sessions appearing on the interview page.
                ...
                Swal.fire({
                    title: 'Error',
                    text: 'Failed to initialize interview session. Please refresh.',
                    icon: 'error',
        ...
        async startInterview() {
            if (!this.logged) {
                Swal.fire({
                    ...
                    text: 'Please login to start an interview.',
                    icon: 'warning',
                    confirmButtonColor: '#8b5cf6'
                });
                return;
            ...
            if (this.interviewAttempts <= 0) {
                Swal.fire({
                    title: 'Limit Reached',
                    text: 'You have no interview sessions remaining today.',
                    icon: 'warning',
                    confirmButtonColor: '#8b5cf6'
                });
                return;
            ...
            try {
                ...
                Swal.fire({
                    title: 'Error',
                    text: 'Failed to start interview session.',
                    icon: 'error',
                    confirmButtonColor: '#8b5cf6'
                });
            } finally {
        ...
        selectPreferredVoice(voices, gender) {
            ...
            if (isMale) {
                ...
                // 2. Fallback to any voice with "Male" or "Guy" in the name
                const anyMale = englishVoices.find(v => /male|guy/i.test(v.name));
                ...
                const femaleNames = ['Zira', 'Susan', 'Hazel', 'Heera', 'Google UK English Female', 'Microsoft Zira', 'Microsoft Hazel', 'Microsoft Susan', 'Microsoft Heera'];
                for (const name of femaleNames) {
                    const voice = englishVoices.find(v => v.name.includes(name));
                    if (voice) return voice;
                }

                const anyFemale = englishVoices.find(v => /female|woman/i.test(v.name));
