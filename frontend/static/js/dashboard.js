  ...
  Alpine.data('dashboard', () => {
    ...
    return {
      ...
      async submitManualProfile() {
        ...
        try {
          const response = await fetch(icp.apiUrl('/api/resume/manual-upload'), {
            method: 'POST',
            headers: {
      ...
      async initDashboard() {
        ...
        try {
          ...
          await Promise.allSettled([
            (async () => {
              try {
                const r = await fetch(icp.apiUrl('/api/interview/history'), { headers: { 'Authorization': 'Bearer ' + token } });
                if (r.status === 200) {
            ...
            (async () => {
              try {
                const r = await fetch(icp.apiUrl('/api/resume/limits'), { headers: { 'Authorization': 'Bearer ' + token } });
                if (r.status === 200) {
                  const j = await r.json();
      ...
      async uploadResume() {
        ...
        try {
          const token = window.icp ? window.icp.state.token : localStorage.getItem("token");
          const r = await fetch(icp.apiUrl('/api/resume/upload'), {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + token },
            body: fd
          });
