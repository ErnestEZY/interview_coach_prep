...
async def delete_account(
    ...
    Data deleted (in order):
      1. GridFS resume files (binary file bytes)
      2. resumes collection documents
      3. interviews collection documents
      4. reset_tokens by email
      5. pending_users by email
      6. audit_logs by user_id
      7. users document

    This is a hard delete — no soft delete, no cache, no recovery.
    Confirmed by requiring the user to type: delete {their_email}
    """
    ...
    if confirmation != expected:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Confirmation text does not match. Please type exactly: delete {current['email']}"
        )
    ...
    for r in resume_docs:
        ...
        if fid:
            try:
                ...
                pass  # File may already be missing — do not block deletion

    # ── 2. Delete resume documents ────────────────────────────────────────────
    ...
    return {
        "message": "Your account and all associated data have been permanently deleted. "
                   "No data is retained after this action."
    }
